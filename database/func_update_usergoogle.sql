...
CREATE OR REPLACE FUNCTION public.update_usergoogle(
	user_id integer,
...
BEGIN
    ...
    WHERE "ID" = user_id;
	RETURN QUERY SELECT "ID" FROM "UsersGoogle" WHERE "ID" = user_id;
END;
$BODY$;

ALTER FUNCTION public.update_user(integer, text, text, text)
    OWNER TO postgres;
